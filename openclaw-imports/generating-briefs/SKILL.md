...
2. **Main agent (browser):** arXiv cs.AI pastweek extraction via `browser_navigate` + `browser_console` IIFE with `dl.children` DOM traversal. Browser_console *usually* works but can fail intermittently on pages with 1,600+ entries (DOM may be inaccessible even though the snapshot shows content — observed May 22, 2026 on cs.AI). **Default to Python urllib for arXiv extraction** unless browser_console is confirmed working. Do cs.AI first, then cs.LG with keyword filtering, then cs.SE if time permits. For cs.LG and cs.SE, apply client-side keyword scoring to filter AI/LLM-relevant papers.
...
- **arXiv doesn't update on weekends** — if running on Saturday/Sunday/Monday, the listings will be from the previous Friday. The page heading shows the exact listing date; always check it for freshness filtering. **Saturday/Sunday formatting:** use "Weekend edition" in the subtitle, note that arXiv is from Friday, and keep the freshness window at 24 hours from the most recent listing date. Friday papers are still valid for a Saturday brief — do not skip arXiv just because it's a weekend.
...
- **process('log') offset parameter is unreliable for reading background RSS output** — `process log offset=0 limit=N` shows the END of the log, not the beginning. `process log offset=N` with a non-zero offset errors with "Unknown process action." And `poll` only gives a truncated preview. **Reliable fallback:** re-run the RSS batch script in foreground via `execute_code` with `subprocess.run()`. This takes ~20 seconds but guarantees clean, complete JSON output every time. The background run still serves its purpose (parallelism), but don't count on being able to read its full output afterward — use it as a best-effort, then re-run foreground if you need the data.
